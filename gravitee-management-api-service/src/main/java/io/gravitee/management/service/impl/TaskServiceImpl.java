...
import static io.gravitee.management.model.permissions.ApiPermission.DEFINITION;
import static io.gravitee.management.model.permissions.ApiPermission.SUBSCRIPTION;
import static io.gravitee.management.model.permissions.ManagementPermission.REVIEWS;
...
public class TaskServiceImpl extends AbstractService implements TaskService {
    ...
    public List<TaskEntity> findAll(String userId) {
        ...
        try {
            ...
            Set<String> apiIds = getApisForAPermission(userId, SUBSCRIPTION.getName(), RoleScope.API);
            ...
            apiIds = getApisForAPermission(userId, REVIEWS.getName(), RoleScope.MANAGEMENT);
            if (!apiIds.isEmpty()) {
            ...
            apiIds = getApisForAPermission(userId, DEFINITION.getName(), RoleScope.API);
            if (!apiIds.isEmpty()) {
                apiIds.forEach(apiId -> {
                    final List<Workflow> workflows = workflowService
                            .findByReferenceAndType(API, apiId, WorkflowType.REVIEW);
    ...
    private Set<String> getApisForAPermission(final String userId, final String permission, final RoleScope roleScope) throws TechnicalException {
        ...
        for (Membership membership : memberships) {
            ...
            if (roleName != null) {
                ...
                if (roleEntity == null) {
                    try {
                        roleEntity = roleService.findById(roleScope, roleName);
